...
const useStyles = makeStyles({
  ...
  // 位置（translate）由 JS 写入——顶层条目与子导航缩进不同，需 X+Y 双轴
  // 位移；默认隐藏，定位后显示。绝对定位子项不参与 flex/gap 布局。
  // zIndex 提层：条目是定位元素时按 DOM 序会盖住先行声明的指示条
  // （选中底色压住竖条），显式置顶（.item 已不再携带定位样式，此处
  // 仍保留作为防御）
  indicator: {
    position: "absolute",
    zIndex: 1,
...
export function Sidebar({ projectId }: { projectId: string | null }) {
  ...
  // 用户指令：与第一层动画一致）。选中态只落在最内层当前项——项目行
  // 无选中态，aria-current 仅出现在顶层「总览」与子导航项上。
  // projects 数据就位后激活链接才出现，故入 deps。
  // 竖条纵向居中（(条目高-16)/2）、横向对齐条目左缘，顶层条目与子
  // 导航缩进不同，X+Y 双轴位移。
  useLayoutEffect(() => {
  ...
  return (
    <aside ref={asideRef} className={styles.root} aria-label={t("nav.projects")}>
      ...
      {(projects ?? []).map((p) => {
        ...
        return (
          <div key={p.id}>
            {/* 项目行无选中态（2026-09-01 用户指令）：仅作展开子导航的入口，
                选中底色/竖条只落在最内层当前项 */}
            <Link to={base} className={styles.item}>
