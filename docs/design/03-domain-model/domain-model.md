...
- 关键属性：类型（13 种前缀之一，创建后不可变——编号前缀与类型绑定）、标题、Markdown 正文、结构化元数据（优先级、备注等辅助信息）、状态、当前修订号、superseded_by
- 生命周期：非 TASK 条目走条目状态机（草稿 → 评审中 → 已确认，双终态已取消/已替代）；TASK 条目走任务状态机（DOM-006）；M1 无条目级物理删除——作废只能取消或替代（BR-003 及"暂无删除入口"的现状）
...
- 身份：（条目，修订号），修订号单调递增
- 关键属性：修订号、时间、变更者（M1 为本地用户或 Agent 会话标识）、内容快照、变更摘要（含状态迁移前后）
