...
classDiagram
    ...
    Project "1" o-- "0..4" ProjectDoc : 拥有（key 受控）
    ...
    note for ProjectDoc "项目级文档不是条目（DOM-009）：无编号、不入关系图、修订同 BR-004 不可变口径"
```
...
- 关键属性：类型（15 种前缀之一，创建后不可变——编号前缀与类型绑定）、标题、Markdown 正文、结构化元数据（优先级、备注等辅助信息）、状态、当前修订号、superseded_by
...
- 修订留痕：2026-09-01 用户指令——UI-xxx 界面设计规格升格为条目类型
  （UI），编号沿用既有 UI-xxx 体系（索引见 05-detailed-design/ui/README.md，
  已被代码注释广泛引用）；类型固定序插入 SEQ 之后、ADR 之前。
  2026-09-04 修订循环——模块设计文档类别条目化为第 15 类型（MOD），
  固定序插入 UI 之后、ADR 之前（与 UI 同属详细设计段）；facilitator 模板
  modules/<short-name>.md「一模块一文档」与「前缀-序号」编号体系同构，
  每模块一条、按类型独立递增。
  条目模型外长文档的纳入方式暂缓（2026-09-02 用户指令回退浏览页，
  重议中，见 open-questions.md OQ-007；综述型单例文档的承载随本改
  落地 DOM-009 项目级文档）
- 假设与开放问题：无
...
- 定义：挂在项目下、按受控 key 索引的单例 Markdown 文档（每项目每 key 恰一篇）；承载「每项目一篇」的综述型内容——项目概览、数据模型、项目结构综述、技术综述
- 身份：（项目，key）；key 取工具级受控词表：`overview`（项目概览）、`data_model`（数据模型）、`structure`（项目结构综述）、`tech_stack`（技术综述）；扩展 key 走设计修订，不允许项目自定义
- 关键属性：key、标题、Markdown 正文、当前修订号
- 生命周期：写入即追加不可变修订，当前内容恒等于最新修订（BR-004 同口径）；随项目级联删除
- 明确不是条目：无显示编号、无状态机、不入关系图（INV-003 两端必须是条目）、不进类型分布；Agent 经 MCP 写、UI 只读渲染（CON-009 同规）
- 写作约束：`tech_stack` 只做栈清单、选型理由摘要与 ADR 索引——技术决策本体唯一以 ADR 条目为事实来源，冲突时以 ADR 为准
- 负责规则：BR-004、FR-001（写入唯一入口为 MCP）、FR-014（导出再生对应模板文件）
- 关系：归属项目；不参与关系图与影响定位
- 来源：2026-09-04 修订循环（facilitator 模板 data-model.md / 项目结构 / 技术综述等单例文档的承载裁决；机制先例为项目概览文档 UI-035 的 project_overview 两表泛化）
- 假设与开放问题：`data_model` / `structure` / `tech_stack` 暂无 UI 浏览入口页（机制先行，入口后置）
...
- **INV-008**：条目类型创建后不可变（编号前缀与类型绑定，2026-08-27 访谈确认；词表扩展不改已建条目类型——2026-09-04 扩至 15 类 +MOD）；
