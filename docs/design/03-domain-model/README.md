...
> 状态：已确认（2026-08-27；2026-09-04 修订循环 +MOD 与 DOM-009，待重新确认）
> 关联：01 需求（已确认）、02 用例（已确认）

## 核心概念

- DOM-001 项目：隔离编号空间与数据的聚合根，级联删除边界；
- DOM-002 条目：设计资产统一对象（类型不可变，单一状态字段按类型选状态机；类型词表 15 种，2026-09-04 +MOD）；
...
- DOM-009 项目级文档：受控 key 索引的单例综述文档（overview/data_model/
  structure/tech_stack），不是条目——无编号、不入关系图、不可变修订
  （2026-09-04 新增）。
