...
| --- | --- | --- | --- | --- |
| 创建 → 草稿 | 类型合法（14 种前缀之一），分配编号 | Agent（MCP） | 生成修订 1 | 编号冲突由领域分配杜绝；重复创建请求生成两个条目（幂等性由调用方保证） |
...
| 已确认 → 已废弃 | 需求失效且无后继，调用显式携带确认参数（2026-09-02 用户指令新增） | Agent（MCP） | 追加修订；条目保持可查询 | 缺确认参数拒绝；终态不可再迁移 |
| 已确认 → 评审中 | 标题或正文被修改（仅内容触发，元数据修改不触发） | Agent（MCP） | 同一事务内：新修订 + 状态退回 | 期望修订号冲突拒绝（BR-005/BR-009） |
| 已取消/已替代/已废弃 → 任意 | 终态，禁止一切迁移与内容编辑 | — | — | 拒绝并返回终态原因（UC-004 A2） |

备注：

- 三个终态的语义分工：已取消 = 未确认即放弃；已替代 = 确认后退役且有后继
  （superseded_by 指向后继）；已废弃 = 确认后失效且无后继（对应 ADR/RFC
  惯例的 deprecated 与 superseded 之分）；
- 不设"已废弃 → 已替代"迁移（2026-09-02 用户指令，终态间不加后继补记）；
  事后确有覆盖关系时用 relates 关系表达；
