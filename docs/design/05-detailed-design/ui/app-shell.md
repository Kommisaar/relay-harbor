...
- **第二层（项目导航栏，停靠约 232px，常驻）**：进入式两级呈现
  ...
  - **进入层级**（`/projects/:id/*`）：顶部**返回行**（左箭头图标 +
    ...
    `stats`，新增项目概览接管 index）/ 数据模型/项目结构/技术综述
    三直达链接（2026-09-04 同日用户指令补入口、随即二次指令定案
    「直接跟在项目统计」：**不单独成组**、无组头，`docs/:key`，
    DOM-009 受控 key；链接常驻，无文档 key
    由页内 DOC_NOT_FOUND 错误态呈现，与空类型常驻同策略）/ 15 类型
    固定清单（2026-09-04 修订循环 +MOD；每类型独立
  ...
  **类型清单分组（2026-09-02 用户指令；2026-09-04 修订循环 +MOD
  至 15 类型）**：类型按设计阶段
  （与 docs/design/ 目录 00→06 同构）以组头小字分隔——需求
  （FR/NFR/BR/CON）、用例（UC）、领域模型（DOM）、架构
  （CMP/INT/SEQ/ADR）、详细设计（UI/MOD）、验证（RISK/OQ）；TASK 与
  任务面板不入组置尾（用户指令）；组头为非交互小字行，不带计数，
  空组恒显示（与无条目类型常驻同策略，结构稳定）；组词表为工具
  级静态常量（随 INV-008 类型集合），非项目数据。MOD（模块设计）
  2026-09-04 修订循环新增，入详细设计组、固定序 UI 之后
  （DOM-002 留痕）。
...
flowchart LR
    ...
    plist -->|点击项目进入| work["/projects/:id/*<br/>项目概览·项目统计·项目文档×3·类型×15·任务面板<br/>第二层=返回行·项目题行·子导航"]
    work -->|返回行| plist
