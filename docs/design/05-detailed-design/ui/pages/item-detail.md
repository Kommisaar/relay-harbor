...
5. **修订历史浮动胶囊**（UI-017，2026-09-02 用户指令由文末区块改浮动
   胶囊）：sticky 悬浮页顶右侧、与页顶保持设定的最小距离（滚动钉在
   固定高度），收起 = 历史图标 + 「修订历史」
   + 修订数，点击展开面板 = 修订号 + 时间 + 操作者 + 摘要（含状态
   迁移记录）；当前版本高亮；**点选历史版本时正文区切换显示该版本
   完整快照**（content_snapshot，只读），头部出现「正在查看 rN
   （历史版本）」提示条与「回到当前」动作；选中后收起、点外/Esc
   关闭（详见 patterns.md「修订时间线」）；无 diff 视图（M1 不引入
   diff 组件）。

```text
← 返回 · relay-harbor / FR 功能需求        (⏱修订历史·3)
FR-001  用户登录              [已确认]
优先级 P1 · r3 · 2 分钟前
────────────────────────────
## 需求描述（Markdown 渲染）
