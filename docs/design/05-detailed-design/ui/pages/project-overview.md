...
2. **修订历史浮动胶囊**（共享 RevisionTimeline，2026-09-02 用户指令改
   ...
   胶囊面板」）；查看历史版自动显示与上一版的单栏 diff（2026-09-03
   用户指令，patterns.md「修订对比」；最旧版本显示该版快照正文）；
...
4. **正文**：Markdown 只读渲染（共享 MarkdownBody，react-markdown +
   ...
   切换历史版本时：有上一版显示与上一版的单栏 diff（共享
   MarkdownDiffBody，patterns.md「修订对比」），最旧版本显示该版快照
   正文，头部元信息不变。
...
快照变化（历史版为与上一版 diff，最旧版为快照正文，patterns.md
「修订对比」）、面板选中后收起、回到当前恢复；滚动时胶囊保持最小距离
钉在固定高度；
