...
2. **修订历史浮动胶囊**（共享 RevisionTimeline，2026-09-02 用户指令改
   浮动胶囊）：sticky 悬浮文档容器顶部右侧、与页顶保持设定的最小
   距离（不占正文文档流，滚动钉在固定高度）；收起 =
   历史图标 + 「修订历史」+ 修订数，点击展开面板（行 = 修订号 +
   摘要 · 操作者 + 相对时间，当前版高亮），点选历史版切正文、
   点当前版回到当前，选中后收起、点外/Esc 关闭（详见 patterns.md
   「修订时间线」）；无 diff 视图（UI-018 同策略）；版本切换为组件
