...
- 路由集中式（`app/router.tsx`，2026-09-01 修订；2026-09-02 拆分）：
  ...
  （项目统计）、`docs/:key`（项目级文档其余三 key，2026-09-04 同日
  用户指令补入口，DOM-009 受控 key、词表外回落 index）、`items`
  （重定向首个类型页）、
  `items/type/:type`、`items/:code`、`tasks`）、`/settings`；
  条目按类型拆独立子页面、聚合页取消（2026-09-01 用户指令；
  同日二次指令：UI-xxx 升格；2026-09-04 修订循环：类型 14→15 +MOD，
  `items/type/MOD` 随常量自动生效）；impact 为详情内嵌区
  （UI-018），不独立路由；导出无路由（项目列表卡片 Popover，
  2026-08-28 用户指令）；搜索无路由（2026-08-28 用户指令暂缓，
  旧地址通配回落 `/projects`）。
