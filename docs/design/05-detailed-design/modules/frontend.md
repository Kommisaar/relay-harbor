...
`search`、`export`、`settings`；每 feature 固定五件套
...
- 路由集中式（`app/router.tsx`，2026-08-28 修订）：`/projects`、
  `/export`（全部项目导出，UI-033）、
  `/projects/:id/`（index=概览；`items`、`items/:code`、`tasks`、
  `search`、`export`）、`/settings`；impact 为详情内嵌区（UI-018），
  不独立路由。
