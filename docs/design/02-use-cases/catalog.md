...
- 状态：草稿
- 主要参与者：ACT-002
- 目标：维护项目下按受控 key 索引的单例综述文档（概览/数据模型/项目结构/技术综述），使综述内容进入管理平面
- 触发：Agent 调用项目级文档读写工具（按 key 定位，携带期望修订号写入）
- 优先级：应该
- 关联需求：FR-019、DOM-009、BR-004
- 详细规约：无需独立规约（乐观并发与不可变修订复用条目同款机制；与条目的差异仅在于无编号、无状态、不入关系图）
- 备注：2026-09-04 修订循环新增（编号接续 UC-018，归属 Agent 写入组）
